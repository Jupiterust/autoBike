...
extern uint8_t vp_rxbuff[VALUEPACK_BUFFER_SIZE];// 用于环形缓冲区的数组，环形缓冲区的大小可以在.h文件中定义VALUEPACK_BUFFER_SIZE
extern uint16_t vp_circle_rx_index;// 数据包环形缓冲区计数
...
void sendValuePack(TxPack *tx_pack_ptr);// 发送数据包
void uart_data_treating(void);//放while使用数据
