...
void Rx_pack(void)
{
    txet_bool = rx_pack.bools[0];
	  txet_bool1 = rx_pack.bools[1];
	
    text_char  = rx_pack.bytes[0];
    txet_short = rx_pack.shorts[0];
    txet_int   = rx_pack.integers[0];
    txet_float = rx_pack.floats[0];
...
void Tx_pack(void)
{
		...
		tx_pack.bools[0] = Tx_Buf[0];
		tx_pack.bools[1] = Tx_Buf[1];
		tx_pack.bools[2] = Tx_Buf[2];
		tx_pack.bools[3] = Tx_Buf[3];
		tx_pack.bools[4] = Tx_Buf[4];
		tx_pack.bools[5] = Tx_Buf[5];
		tx_pack.bools[6] = Tx_Buf[6];
    tx_pack.bools[7] = Tx_Buf[9];

		tx_pack.bytes[0] = Tx_Buf[10];
		tx_pack.shorts[0] = Tx_Buf[11];
	  tx_pack.integers[0] = Tx_Buf[12];
	  tx_pack.floats[0] = Tx_Buf[13];
...
void send_ValuePack(Tx_Pack *tx_pack_ptr)
{
  ...
  vp_txbuff[0]=0xa5;//包头
    ...
    Serial1_SendArray(USART1,(uint8_t *)vp_txbuff,TX_PACK_BYTE_SIZE+3);
}
